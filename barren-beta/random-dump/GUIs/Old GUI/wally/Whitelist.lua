local key = "oXW5zs*MAH4jXUHAErqcBQB6LCf#PG0@kHCRFEI97Vlu7rxK8DdJC9832#ph&dHG5*QAZcI#HAx6yQOHT9PAU83SnifEgvwiwNz"
local hashlevel = 10
local url = "https://pastebin.com/raw/iuSaCL3L"
function SHA256(mes)
...
local ipcheck = ip
local webkeys = game:HttpGet(url, true)
local vault = SHA256(ip .. key .. "eon_tranquility")
local whitelistkey = levels(vault)

if ip ~= ipcheck then
	game.Players.LocalPlayer:Kick("IP Spoofing Detected! User kicked. Please purchase the gui... 🙃")
	return
end

if url ~= urlcheck then
	game.Players.LocalPlayer:Kick("URL Spoofing Detected! User kicked. Please purchase the gui... 🙃")
	return
end

if string.find(webkeys,whitelistkey) then
	print("Whitelist verified!")
	-- YOUR SCRIPT GOES HERE!
else
	print("Whitelist rejected!")
	game.Players.LocalPlayer:Kick("You are not whitelisted! Please purchase the gui... 🙃")
	return
end