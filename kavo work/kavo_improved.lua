...
function kavo:Create(class, properties)
...
function kavo:TweenObject(obj, properties, duration, ...)
    kavo.gs["TweenService"]:Create(obj, TweenInfo.new(duration, ...), properties):Play()
end
...
function kavo.new(kavName, themeList)
    ...
    self2.ChangeToggleKey = function(NewKey)
        ...
        kavData.UpConnection = kavo.gs["UserInputService"].InputEnded:Connect(function(Input)
            if Input.KeyCode == kavData.ToggleKey and not typing then
                toggled = not toggled
                if not toggled then
                    local Tween = kavo.gs["TweenService"]:Create(self2.container, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, 0, 0, 0)})
                    Tween:Play()
                    Tween.Completed:Wait()
                    self2.userinterface.Enabled = false
                else
                    self2.userinterface.Enabled = true
                    local Tween = kavo.gs["TweenService"]:Create(self2.container, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, 525, 0, 318)})
                    Tween:Play()
                    Tween.Completed:Wait()
                end
            end
    ...
    self2.ToggleUi = function()
        if kavo.gs["CoreGui"][LibName].Enabled then
            toggled = false
            local min = kavo.gs["TweenService"]:Create(self2.container, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, 0, 0, 0)})
            min:Play()
            min.Completed:Wait()
            self2.userinterface.Enabled = false
        else
            toggled = true
            self2.userinterface.Enabled = true
            local max = kavo.gs["TweenService"]:Create(self2.container, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, 525, 0, 318)})
            max:Play()
            max.Completed:Wait()
        end
    end
    ...
    self2.containercorner = self:Create("UICorner", {
        Name = "ContainerCorner",
        CornerRadius = UDim.new(0, 4),
    })
    self2.containercorner.Parent = self2.container
    ...
    self2.headercorner = self:Create("UICorner", {
        Name = "HeaderCorner",
        CornerRadius = UDim.new(0, 4),
    })
    self2.headercorner.Parent = self2.header
    ...
    function kavo.settitle(name)
        self2.title.Text = tostring(name)
    end
    ...
    self2.headercover = self:Create("Frame", {
        Name = "HeaderCover",
        BackgroundColor3 = themeList.Header,
        BorderSizePixel = 0,
    ...
    self2.headercover.Parent = self2.header 
    ...
    self2.sidebarcorner = self:Create("UICorner", {
        Name = "SideBarCorner",
        CornerRadius = UDim.new(0, 4),
    })
    self2.sidebarcorner.Parent = self2.sidebar
    ...
    self2.categorylayout = self:Create("UIListLayout", {
        Name = "CategoryLayout",
        SortOrder = Enum.SortOrder.LayoutOrder
    })
    self2.categorylayout.Parent = self2.categories

    self2.sidebarcover = self:Create("Frame", {
        Name = "SideBarCover",
        BackgroundColor3 = themeList.Header,
        BorderSizePixel = 0,
    ...
    self2.sidebarcover.Parent = self2.sidebar
    ...
    function self2:Category(name)
        ...
        category.pagelayout = kavo:Create("UIListLayout", {
            Name = "PageLayout",
            Padding = UDim.new(0, 5),
            SortOrder = Enum.SortOrder.LayoutOrder
        })
        category.pagelayout.Parent = category.page
        ...
        category.categorycorner = kavo:Create("UICorner", {
            Name = "CategoryButtonCorner",
            CornerRadius = UDim.new(0, 5)
        })
        category.categorycorner.Parent = category.button

        local function UpdateSize()
            local cS = category.pagelayout.AbsoluteContentSize

            kavo.gs["TweenService"]:Create(category.page, TweenInfo.new(0.15, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {CanvasSize = UDim2.new(0, cS.X, 0, cS.Y)}):Play()
        end
            ...
            for i, v in next, self2.categories:GetChildren() do
                if v:IsA("TextButton") then
                    if themeList.SchemeColor == Color3.fromRGB(255, 255, 255) then
                        kavo:TweenObject(v, {TextColor3 = Color3.fromRGB(255, 255, 255)}, 0.2)
                    end 
                    if themeList.SchemeColor == Color3.fromRGB(0, 0, 0) then
                        kavo:TweenObject(v, {TextColor3 = Color3.fromRGB(0, 0, 0)}, 0.2)
                    end 
                    kavo:TweenObject(v, {BackgroundTransparency = 1}, 0.2)
                end
            end
            if themeList.SchemeColor == Color3.fromRGB(255, 255, 255) then
                kavo:TweenObject(category.button, {TextColor3 = Color3.fromRGB(0, 0, 0)}, 0.2)
            end 
            if themeList.SchemeColor == Color3.fromRGB(0, 0, 0) then
                kavo:TweenObject(category.button, {TextColor3 = Color3.fromRGB(255, 255, 255)}, 0.2)
            end 
            kavo:TweenObject(category.button, {BackgroundTransparency = 0}, 0.2)
        end)
        ...
        function category:Section(name, hidden)
            ...
            section.pagelayout = kavo:Create("UIListLayout", {
                Name = "SectionLayout",
                Padding = UDim.new(0, 5),
                SortOrder = Enum.SortOrder.LayoutOrder
            })
            section.pagelayout.Parent = section.page
            ...
            section.headercorner = kavo:Create("UICorner", {
                Name = "HeaderCorner",
                CornerRadius = UDim.new(0, 4)
            })
            section.headercorner.Parent = section.header
            ...
            section.containerlayout = kavo:Create("UIListLayout", {
                Name = "SectionContainerLayout",
                Padding = UDim.new(0, 3),
                SortOrder = Enum.SortOrder.LayoutOrder
            })
            section.containerlayout.Parent = section.container

            local function UpdateSectionFrame()
                local innerSc = section.containerlayout.AbsoluteContentSize
                section.container.Size = UDim2.new(1, 0, 0, innerSc.Y)
                local frameSc = section.pagelayout.AbsoluteContentSize
            ...
            if themeList.SchemeColor == Color3.fromRGB(255,255,255) then
                kavo:TweenObject(section.name, {TextColor3 = Color3.fromRGB(0,0,0)}, 0.2)
            end
            if themeList.SchemeColor == Color3.fromRGB(0,0,0) then
                kavo:TweenObject(section.name, {TextColor3 = Color3.fromRGB(255,255,255)}, 0.2)
            end
            ...
            function section:Cheat(kind, name, tip, callback, data)
                ...
                cheat.informationcorner = kavo:Create("UICorner", {
                ...
                if kind then
                    if string.lower(kind) == "toggle" then
                        ...
                        if themeList.SchemeColor == Color3.fromRGB(255, 255, 255) then
                            kavo:TweenObject(cheat.information, {TextColor3 = Color3.fromRGB(0, 0, 0)}, 0.2)
                        end 
                        if themeList.SchemeColor == Color3.fromRGB(0, 0, 0) then
                            kavo:TweenObject(cheat.information, {TextColor3 = Color3.fromRGB(255, 255, 255)}, 0.2)
                        end
                        ...
                        cheat.toggle.MouseButton1Click:Connect(function()
                            if not focusing then
                                ...
                                if cheat.value then
                                    kavo.gs["TweenService"]:Create(cheat.toggleenabled, TweenInfo.new(0.11, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {ImageTransparency = 0}):Play()
                                    ...
                                    kavo.gs["TweenService"]:Create(cheat.toggleenabled, TweenInfo.new(0.11, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {ImageTransparency = 1}):Play()
                                    local clone = cheat.toggleanimation:Clone()
                                    clone.Parent = cheat.toggle
                                    local x, y = (mouse.X - clone.AbsolutePosition.X), (mouse.Y - clone.AbsolutePosition.Y)
                                    clone.Position = UDim2.new(0, x, 0, y)
                                ...
                                for i, v in next, self2.infocontainer:GetChildren() do
                                    kavo:TweenObject(v, {Position = UDim2.new(0, 0, 2, 0)}, 0.2)
                                    focusing = false
                                end
                                kavo:TweenObject(self2.blurframe, {BackgroundTransparency = 1}, 0.2)
                            end
                        end)
                        ...
                        cheat.toggle.MouseEnter:Connect(function()
                            if not focusing then
                                kavo.gs["TweenService"]:Create(cheat.toggle, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {BackgroundColor3 = Color3.fromRGB(themeList.ElementColor.r * 255 + 8, themeList.ElementColor.g * 255 + 9, themeList.ElementColor.b * 255 + 10)}):Play()
                                hovering = true
                            end
                        end)
                        ...
                        cheat.toggle.MouseLeave:Connect(function()
                            if not focusing then
                                kavo.gs["TweenService"]:Create(cheat.toggle, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {BackgroundColor3 = themeList.ElementColor}):Play()
                                hovering = false
                            end
                        end)
                        ...
                        cheat.informationbutton.MouseButton1Click:Connect(function() 
                            if not viewde then
                                ...
                                kavo:TweenObject(cheat.information, {Position = UDim2.new(0, 0, 0, 0)}, 0.2)
                                kavo:TweenObject(self2.blurframe, {BackgroundTransparency = 0.5}, 0.2)
                                kavo:TweenObject(cheat.toggle, {BackgroundColor3 = themeList.ElementColor}, 0.2)
                                wait(1.5)
                                focusing = false
                                kavo:TweenObject(cheat.information, {Position = UDim2.new(0, 0, 2, 0)}, 0.2)
                                kavo:TweenObject(self2.blurframe, {BackgroundTransparency = 1}, 0.2)
                                wait()
                                viewde = false
                            end
                    ...
                    elseif string.lower(kind) == "colorpicker" then
                        ...
                        cheat.informationbutton.MouseButton1Click:Connect(function() 
                            if not viewde then
                                ...
                                kavo:TweenObject(cheat.information, {Position = UDim2.new(0, 0, 0, 0)}, 0.2)
                                kavo:TweenObject(self2.blurframe, {BackgroundTransparency = 0.5}, 0.2)
                                kavo:TweenObject(cheat.colorpicker, {BackgroundColor3 = themeList.ElementColor}, 0.2)
                                wait(1.5)
                                focusing = false
                                kavo:TweenObject(cheat.information, {Position = UDim2.new(0, 0, 2, 0)}, 0.2)
                                kavo:TweenObject(self2.blurframe, {BackgroundTransparency = 1}, 0.2)
                    ...
                    elseif string.lower(kind) == "button"then
                        ...
                        cheat.informationbutton.MouseButton1Click:Connect(function() 
                            if not viewde then
                                ...
                                kavo:TweenObject(cheat.information, {Position = UDim2.new(0, 0, 0, 0)}, 0.2)
                                kavo:TweenObject(self2.blurframe, {BackgroundTransparency = 0.5}, 0.2)
                                kavo:TweenObject(cheat.button, {BackgroundColor3 = themeList.ElementColor}, 0.2)
                                wait(1.5)
                                focusing = false
                                kavo:TweenObject(cheat.information, {Position = UDim2.new(0, 0, 2, 0)}, 0.2)
                                kavo:TweenObject(self2.blurframe, {BackgroundTransparency = 1}, 0.2)
